...
STATUS_FILE_PATH = os.path.join(SCRIPT_DIR, "crawler.status")
OUTPUT_FILE_NAME = "latest_crawl_output.csv"
...
def _write_status(status, message, pid=None):
    try:
        current_data = {}
        if pid is None and os.path.exists(STATUS_FILE_PATH):
            try:
        ...
        new_data = {
            "status": status,
            "message": message,
            "pid": pid or current_data.get('pid')
        }

        if status in ['complete', 'error', 'idle']:
            new_data.pop('pid', None)

        with open(STATUS_FILE_PATH, 'w', encoding='utf-8') as f:
            json.dump(new_data, f, ensure_ascii=False, indent=2)

        print(f"[Status Update] {status}: {message} (PID: {new_data.get('pid')})")

    except Exception as e:
        print(f"!! 严重错误：无法写入状态文件 {STATUS_FILE_PATH} !! - {e}")
...
async def main():
    all_final_data = []
    print("--- 启动一体化爬取任务 ---")
    print(f"--- 目标 URL: {BRAND_URLS} ---")

    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        page = await browser.new_page()
        for i, brand_url in enumerate(BRAND_URLS):
            _write_status("running", f"正在处理第 {i + 1}/{len(BRAND_URLS)} 个品牌: {brand_url}...", pid=os.getpid())
    ...
    _write_status("running", "所有品牌爬取完毕，正在整合数据...")
    ...
    if '评价数量' not in master_df.columns: master_df['评价数量'] = 0
    if '所有评价' not in master_df.columns: master_df['所有评价'] = ''
    if '平均评分' not in master_df.columns: master_df['平均评分'] = np.nan

    master_df['评价数量'] = master_df['评价数量'].fillna(0).astype(int)
    master_df['所有评价'] = master_df['所有评价'].fillna('')
    if '图片链接' not in master_df.columns: master_df['图片链接'] = ''
    master_df['图片链接'] = master_df['图片链接'].fillna('')
    if '品牌' not in master_df.columns: master_df['品牌'] = '未知品牌'
    ...
    if not master_df.empty and '品牌' in master_df.columns:
        unique_brands = list(master_df['品牌'].unique())
        unique_brands.sort()
        brands_joined = "".join(unique_brands).replace(" ", "")
        if not brands_joined: brands_joined = "未知品牌"
    else:
        brands_joined = "未知品牌"

    dynamic_output_file = f"易车_{brands_joined}参数与口碑汇总.csv"

    master_df.to_csv(OUTPUT_FILE_PATH, index=False, encoding='utf-8-sig')

    print(f"\n🎉🎉🎉 一体化爬取与整合任务全部完成！🎉🎉🎉")
    print(f"共生成 {len(master_df)} 条数据记录。")
    print(f"文件内容已保存至固定文件: {OUTPUT_FILE_PATH}")
    print(f"建议下载文件名为: {dynamic_output_file}")

    return dynamic_output_file


if __name__ == "__main__":
    current_pid = os.getpid()
    if len(sys.argv) > 1:
        BRAND_URLS = sys.argv[1:]

    if not BRAND_URLS:
        _write_status("error", "启动失败：未提供任何 URL 参数。", pid=current_pid)
        print("错误：请提供至少一个易车品牌 URL 作为命令行参数。", file=sys.stderr)
        sys.exit(1)

    _write_status("running", f"爬虫任务已启动 (PID: {current_pid})，正在处理 {len(BRAND_URLS)} 个URL...",
                  pid=current_pid)

    try:
        new_filename = asyncio.run(main())

        if new_filename and os.path.exists(OUTPUT_FILE_PATH):

            _write_status("complete", f"爬取成功完成！\n数据已保存为: {new_filename}")
        else:
            _write_status("complete", "任务运行完成，但未爬取到任何新数据。")

    except BaseException as e:
        error_message = f"爬虫任务意外崩溃: {str(e)}\n{traceback.format_exc()}"
        _write_status("error", error_message)
