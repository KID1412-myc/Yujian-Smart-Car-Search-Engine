...
def main():
    try:
        ...
        file_paths_to_import = sys.argv[1:]

        if not file_paths_to_import:
            print(f"错误：请提供至少一个 CSV 文件路径作为命令行参数。")
            print(f"用法: python {sys.argv[0]} <文件1.csv> <文件2.csv> ...")
            return

        print(f"将要导入以下 {len(file_paths_to_import)} 个文件:")
        for f in file_paths_to_import:
            print(f"  - {f}")
        print("-" * 30)
        ...
        for file_path in file_paths_to_import:
